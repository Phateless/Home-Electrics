...
void Start_Routine(void){
  ...
  // PRINT 30 SEC COUNTDOWN
    ...
    lcd.setCursor(7, 0);
    for(int i=4;i>= 1;i--) // Countdown Setting
...
void if_air_off()
{
  ...
  //WAIT 10 SECONDS FOR THE AIR PIN 9 TO COME ON
    int count = 0;
    while((digitalRead(Air) == LOW) && (count <= 20) ){
       light_control();
      tone(buzzer, 500);
       for(int i=0; i<5 ;i++){
        light_control();
...
{  
  ...
  //WAIT 10 SECONDS FOR THE WATER PIN 10 TO COME ON
    int count = 0;
    while((digitalRead(Water) == LOW) && (count <= 20) ){  
      light_control();
      tone(buzzer, 500);
       for(int i=0; i<5 ;i++){
        light_control();
...
light_control();
  ...
  // PRINT * MINUTE COUNTDOWN
    ...
    int counter = 0;
    int j=5;
    int min = 1; // Countdown Setting ++++
  

      lcd.setCursor(7, 0);
      lcd.print(min);
    for(int i=(1*600);i> 0;i--) //1 min = 600*100ms =60000ms = 60sec  // Countdown Setting ++++
    {  
      light_control();
      Emergency_ButtonState = digitalRead(Emergency);
      if(Emergency_ButtonState == HIGH)Emergency_Routine_2();
