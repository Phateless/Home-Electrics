...
void Start_Routine(void) {
  ...
  lcd.setCursor(7, 0);
  for (int i = 4; i >= 0; i-- ) // Countdown Setting      *************Startup Timing*********30 Seconds*********
...
void Stop_Routine(void) {
  ...
  int counter = 0;
  int j = 5;
  int min = 1; // ***********Countdown Setting************15 mins******
  ...
  Serial.println(min);
  /* Modified section */
  for (int i = 0; i < 1; i++) //15min  // *************Countdown Setting*************15 mins****
  {



    for (int i = 0; i < 600; i++) //1 min = 60*100ms =60000ms = 60sec 
    {
      light_control();
      Emergency_ButtonState = digitalRead(Emergency);
      if (Emergency_ButtonState == HIGH)Emergency_Routine_2();
  ...
  /* //Commented code below is initial code
    for (int i = (1 * 9000); i > 0; i--) //1 min = 600*100ms =60000ms = 60sec  // *************Countdown Setting*************15 mins****
    {
     //Added code
     light_control();
     Emergency_ButtonState = digitalRead(Emergency);
